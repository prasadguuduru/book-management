...
# MCP Proxy Triage Notes
pkill -f "node mcp-proxy.js"
nohup node mcp-proxy.js > mcp-proxy.log 2>&1 &

sleep 2 && curl -X POST http://localhost:3000 -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d '{"jsonrpc": "2.0", "method": "ping", "id": 1}' -w "\nStatus: %{http_code}\n"


curl -X POST http://localhost:3000 -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d '{"jsonrpc": "2.0", "method": "tools/list", "id": 1}' -w "\nStatus: %{http_code}\n"
{"result":{"tools":[{"name":"ping","inputSchema":{"type":"object","properties":
{}}}]},"jsonrpc":"2.0","id":1}                                  
#  test the ping too
curl -X POST http://localhost:3000 -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d '{"jsonrpc": "2.0", "method": "tools/call", "params": {"name": "ping", "arguments": {}}, "id": 1}' -w "\nStatus: %{http_code}\n"
