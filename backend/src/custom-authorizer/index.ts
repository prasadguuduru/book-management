 ...
 * Self-contained implementation to avoid dependency issues
 */
...
// User roles enum
type UserRole = 'AUTHOR' | 'EDITOR' | 'PUBLISHER' | 'READER';

// Simple logger for the authorizer
const logger = {
  info: (message: string, data?: any) => console.log(`[INFO] ${message}`, data ? JSON.stringify(data) : ''),
  error: (message: string, error: Error, data?: any) => console.error(`[ERROR] ${message}`, error.message, data ? JSON.stringify(data) : ''),
  security: (message: string, data?: any) => console.log(`[SECURITY] ${message}`, data ? JSON.stringify(data) : ''),
  functionEntry: (fn: string, data?: any, context?: any) => console.log(`[ENTRY] ${fn}`, data ? JSON.stringify(data) : '', context ? JSON.stringify(context) : '')
};
...
export const handler = async (
  ...
  logger.functionEntry('custom-authorizer-handler', {
  ...
  try {
    ...
    if (!jwtSecret && environment !== 'development') {
      logger.error('JWT_SECRET not configured', new Error('Missing JWT configuration'), { correlationId });
    ...
    if (!token) {
      logger.security('Custom authorizer failed - no token provided', {
        methodArn: event.methodArn,
        correlationId
      });
      return generatePolicy('unauthorized', 'Deny', event.methodArn);
    ...
    if (!cleanToken || cleanToken.trim().length === 0) {
      logger.security('Custom authorizer failed - empty token', { correlationId });
      return generatePolicy('unauthorized', 'Deny', event.methodArn);
    }
    ...
    if (!payload.userId || !payload.email || !payload.role) {
      logger.security('Custom authorizer failed - invalid token payload', {
    ...
    if (!validRoles.includes(payload.role as UserRole)) {
      logger.security('Custom authorizer failed - invalid role', {
        role: payload.role,
        correlationId
      });
      return generatePolicy('unauthorized', 'Deny', event.methodArn);
    ...
    logger.security('Custom authorizer token verified', {
    ...
    logger.security('Custom authorizer denied access', {
