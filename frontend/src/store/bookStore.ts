...
import { Book, CreateBookRequest, UpdateBookRequest, WorkflowEntry } from '@/types'
...
export const useBookStore = create<BookStore>((set) => ({
  ...
  updateBook: async (bookData: UpdateBookRequest) => {
    ...
    try {
      ...
      set((state: BookState) => ({
        ...
        currentBook: state.currentBook?.bookId === updatedBook.bookId ? updatedBook : state.currentBook,
        isLoading: false
      }))
  ...
  submitBookForEditing: async (bookId: string) => {
    ...
    try {
      ...
      set((state: BookState) => ({
        ...
        currentBook: state.currentBook?.bookId === bookId ? updatedBook : state.currentBook,
        isLoading: false
      }))
      
      return updatedBook
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to submit book'
      set({ error: errorMessage, isLoading: false })
      throw new Error(errorMessage)
    }
  },
  ...
  approveBook: async (bookId: string, comments?: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to approve book'
      set({ error: errorMessage, isLoading: false })
      throw new Error(errorMessage)
    }
  },
  ...
  rejectBook: async (bookId: string, comments: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to reject book'
      set({ error: errorMessage, isLoading: false })
      throw new Error(errorMessage)
    }
  },
  ...
  publishBook: async (bookId: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to publish book'
      set({ error: errorMessage, isLoading: false })
      throw new Error(errorMessage)
    }
  },
