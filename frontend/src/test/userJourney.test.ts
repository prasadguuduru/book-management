...
describe('Complete User Journey', () => {
  beforeEach(() => {
    ...
    const mockUser = mockApiService.getMockUsers().find(u => u.role === 'AUTHOR')
    if (mockUser) {
  ...
  it('should complete the full book publishing workflow', async () => {
    ...
    const submittedBook = await mockApiService.submitBookForEditing(createdBook.bookId)
    expect(submittedBook.status).toBe('SUBMITTED_FOR_EDITING')

    // 3. Editor approves the book
    const approvedBook = await mockApiService.approveBook(createdBook.bookId, 'Great work!')
  ...
  it('should filter books by status correctly', async () => {
    ...
    const submittedBooks = await mockApiService.getBooks('SUBMITTED_FOR_EDITING')
